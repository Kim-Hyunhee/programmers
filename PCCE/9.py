'''
9. 동영상 재생기
당신은 동영상 재생기를 만들고 있습니다. 당신의 동영상 재생기는 10초 전으로 이동, 10초 후로 이동, 
...
def solution(video_len, pos, op_start, op_end, commands):
    ...
    answer = time_to_seconds(pos)
    video_length = time_to_seconds(video_len)
    op_start_time = time_to_seconds(op_start)
    op_end_time = time_to_seconds(op_end)

    # 1️⃣ 처음 `pos`가 오프닝 구간이면 `op_end`로 이동
    if op_start_time <= answer <= op_end_time:
        answer = op_end_time

    # 2️⃣ 명령어 실행
    for command in commands:
        if command == "prev":
            answer = max(0, answer - 10)  # 0초 미만 방지
        if command == "next":
            answer = min(video_length, answer + 10)  # 영상 길이 초과 방지

        # 3️⃣ 이동 후, 다시 오프닝 구간인지 체크
        if op_start_time <= answer <= op_end_time:
            answer = op_end_time
